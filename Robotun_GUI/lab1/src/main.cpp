...
// #ifdef DEBUG
// #define TRACE_LOG(format, ...)   printf("%s(%d): " format "\n", __func__, __LINE__, ##__VA_ARGS__)
// #else
...
void CreateControlPanel()
{
    ...
    if (paused)
    {
        if (ImGui::Button("PLAY", ImVec2(90, 30)))
            paused = false;
    }
    else
    {
...
void CreateCanvas()
{
    ...
    if (mass_draw_r < 12.0f)
        mass_draw_r = 12.0f;
    if (mass_draw_r > 40.0f)
        mass_draw_r = 40.0f;
...
int main()
{
    ...
    main_dpi_scale = ImGui_ImplGlfw_GetContentScaleForMonitor(glfwGetPrimaryMonitor());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        current_wheel_x = state.phi * params.R;
        CreateMenu();
